...
- 操作系统：x86-64 架构的任意 Linux 发行版（推荐 Ubuntu 22.04 或更高版本）
  - Windows 用户可使用 WSL 2
  - macOS 用户可使用 [OrbStack](https://orbstack.dev/) 虚拟机或使用我们提供的服务器环境
...
本实验的框架代码使用了C++的一些便利特性来简化内存管理和数据操作。如果你对C++不太熟悉，建议先阅读 **[C程序员的C++实用指南](docs/cpp-for-c-programmers.md)**。这份指南会解释实验中会用到的C++特性，帮助你快速上手，无需学习整个C++语言。
...
> ```
> 
> 使用更高版本的C++标准可以让某些代码写得更简洁（比如C++20的结构化绑定、C++23的`std::print`），但这完全是可选的。实验文档中涉及Modern C++特性的地方都会用提示框标注。
...
你可以阅读[调试指南](docs/debugging-guide.md)，了解如何使用评测脚本和 VSCode 调试。
